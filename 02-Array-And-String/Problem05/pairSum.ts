const pairSum = (numbers: Array<number>, targetSum: number): Array<number> | null => {
    ...
    while(startingPoint <= endingPoint){
        let requiredNumber: number = targetSum - numbers[startingPoint];
        if(NumSet.has(requiredNumber)) return[startingPoint, numbers.indexOf(requiredNumber)];
        ...
        if(NumSet.has(requiredNumber)) return[endingPoint, numbers.indexOf(requiredNumber)];
